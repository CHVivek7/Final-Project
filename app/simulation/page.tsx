...
export default function Home() {
  ...
  const addAtom = () => {
    setAtoms([...atoms, { element: "H", x: 1, y: 1, z: 1 }]);
  };
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/predict/`, {
        ...
        body: JSON.stringify({
          num_atoms: Number(predictionInput.num_atoms || 0),
          num_electrons: Number(predictionInput.num_electrons || 0),
          num_qubits: Number(predictionInput.num_qubits || 0),
          basis_set_size: Number(predictionInput.basis_set_size || 0),
          molecular_complexity: Number(predictionInput.molecular_complexity || 0)
        }),
      });
    ...
    <main className="flex min-h-screen flex-col items-center p-6 md:p-24">
      <div className="z-10 max-w-6xl w-full">
        ...
        <div className="text-foreground/40 text-sm mt-8 justify-center text-center">
          <a className='underline hover:no-underline hover:font-bold' href='https://github.com/Qiskit/textbook/blob/main/notebooks/ch-applications/vqe-molecules.ipynb'>📃</a>
        </div>
      </div>
    </main>
  );
